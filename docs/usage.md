...
しきい値と異なり、この値を変更しただけでは結果には反映されません(サンプリング間隔を変えると取得するフレームそのものが変わるため、動画・GIFの再読み込みが必要になります)。値を変更すると「この間隔で再スキャン」ボタンが有効になるので、反映するにはこのボタンを押してファイルを選び直さずに再スキャンしてください。再スキャンすると、フレームの並びが振り直されるため、手動で切り替えていた採用/除外の状態はリセットされ、新しい間隔での自動判定結果に戻ります。

...
結果一覧ヘッダの「選択したフレームをGIFでダウンロード」ボタン(ZIPボタンの右隣)を押すと、採用中のフレームをタイムスタンプ昇順に1本のアニメーションGIFとしてまとめて書き出せます。生成中はボタンが無効化され、「生成中…(n/総数)」の形式で進捗が表示されます。

GIF出力の設定は、設定パネルの「GIF出力」欄で調整できます。

### フレーム遅延

各フレームの表示時間です(50〜3000ms、既定500ms)。全フレーム共通の固定値になるため、もとの動画・GIFのフレーム間隔の緩急(テンポ)は再現されません。

### 最大幅

出力GIFの最大幅です(320/480/640/960/1280px、またはフル解像度。既定640px)。元の幅がこの値以下の場合は縮小されません。

### 色数

量子化する最大色数です(256/128/64/32色。既定256色)。

GIFは色数の設定にかかわらず最大256色までしか使えない形式のため、フレーム数や解像度によってはファイルサイズが大きくなりやすい点に注意してください。サイズを抑えたい場合は最大幅を小さくするか、色数を減らしてください。

透明画素を含むフレーム(透過GIFの読み込みなど)は自動的に検出され、透過を保持したまま書き出されます。この判定は自動で行われるため、設定項目としては増えません。
...
何ミリ秒おきにフレームを調べるかの設定です(既定200ms)。動画が長く、この間隔だとサンプル数が上限(600サンプル)を超えてしまう場合は、自動的に間隔が広げられます。

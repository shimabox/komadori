...
何ミリ秒おきにフレームを調べるかの設定です(既定200ms)。動画・GIFいずれにも効きます。

対象(動画・GIFとも)の総再生時間が長く、この間隔だとサンプル数が上限(600サンプル)を超えてしまうときは、自動的に間隔が広げられます。またしきい値と異なり、この値を変更しただけでは結果には反映されません(サンプリング間隔を変えると取得するフレームそのものが変わるため、再読み込みが必要になります)。値を変更すると「この間隔で再スキャン」ボタンが有効になるので、反映するにはこのボタンを押してファイルを選び直さずに再スキャンしてください。再スキャンすると、フレームの並びが振り直されるため、手動で切り替えていた採用/除外の状態はリセットされ、新しい間隔での自動判定結果に戻ります。

GIFの場合、実際にサンプリング対象になるのはGIF自身が持つ各フレームの表示時間(ディレイ)に基づく時刻です。ディレイが短いGIF(パラパラ漫画のようなアニメーション等)では、間隔を広めに設定しているとフレーム数が今までより減ることがあります。細かく見たい場合は間隔を下限(10ms)まで下げてください。GIFのディレイは10ms単位まで表現できるため、下限まで下げるとほぼ全フレームが選ばれる状態に戻せます(ただし長いGIFでは、下限まで下げてもサンプル数の上限(600サンプル)の制約で全フレームには戻らないことがあります)。

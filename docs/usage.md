...
GIFの場合、実際にサンプリング対象になるのはGIF自身が持つ各フレームの表示時間(ディレイ)に基づく時刻です。ディレイが短いGIF(パラパラ漫画のようなアニメーション等)では、間隔を広めに設定しているとフレーム数が今までより減ることがあります。細かく見たい場合は間隔を下限(20ms)まで下げてください。ほぼ全フレームが選ばれる状態に戻せます。
